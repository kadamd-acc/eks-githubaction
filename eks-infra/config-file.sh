...
then
   s3=`aws s3 ls s3://landg-terraform-state/eks/configfile/config | awk '{print $4}'`
   echo "config file is available in s3 with filename=" $s3
   if [ "$s3" == "$file_name" ];
   then
      echo "pull file from s3"   
      copy_file=`aws s3 cp s3://landg-terraform-state/eks/configfile/config /home/runner/.kube/`
